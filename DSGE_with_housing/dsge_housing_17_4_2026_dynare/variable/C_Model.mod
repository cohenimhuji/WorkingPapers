...
%shock_j_tag*(p_gamma_tag*h_o_tag^(-teta)/(p_gamma_tag*h_o_tag^(1-teta)+(1-p_gamma_tag)*h_r_tag^(1-teta)))=lambda_tag*q*(1+k)+p_beta_tag*lambda_tag(+1)*q(+1)*(k-1)
%-lagr_mu*ltv*q +p_beta_tag * lagr_mu(+1) * q(+1) * (ltv(+1) - ltv_old(+1));						

shock_j_tag*(p_gamma_tag*h_o_tag^(-teta)/(p_gamma_tag*h_o_tag^(1-teta)+(1-p_gamma_tag)*h_r_tag^(1-teta)))=lambda_tag*q*(1+k)+p_beta_tag*lambda_tag(+1)*q(+1)*(k-1)
-lagr_mu*ltv*q -p_beta_tag * lagr_mu(+1) * q(+1) * ltv(1) *  (1 - alpha) * (ltv_old(+1) - 1);																																					  
...
lagr_mu=lambda_tag-p_beta_tag*lambda_tag(+1)*RL + p_beta_tag * lagr_mu(+1) * (1 - ref - ltv_old * (1 - alpha));

//     (13)  eq. (29) in the pdf file

%B_tag=ltv*q*h_o_tag;
B_tag = (1 - ref) * B_tag(-1) + B_tag_n;
%B_tag_n = ltv * q * (h_o_tag - h_o_tag(-1)) + ltv_old * (q * h_o_tag(-1) - (1 - ref) * B_tag(-1));
B_tag_n = alpha * ltv * q * h_o_tag + (1 - alpha) * (ltv * q * (h_o_tag - h_o_tag(-1)) + ltv_old * (ltv * q * h_o_tag(-1) - B_tag(-1)));
...
Income_poor = q*(h_o_tag(-1) - h_o_tag) + B_tag_n - (RL(-1) - 1 + ref)*B_tag(-1);
...
Y=shock_A*(N^tau)*(N_tag^(1-tau)) - 1 * (h - h(-1)) * 1;
...
% update at : 28.4.2026 after revision 2:
p_m_r_ratio=(eps_r-1)/eps_r+(xi_cost_r/eps_r)*(DP_r)*(DP_r - (DP_TARGET^(1-inert_DP))*DP_r(-1)^inert_DP)-(SDF*(r(+1)/r)*xi_cost_r/eps_r)*DP_r(+1)*(h_inv(+1)/h_inv)*((DP_r(+1) - DP_TARGET^(1-inert_DP)*DP_r^inert_DP));
...
ltv = p_ltv - shock_ltv - shock_ltv_all;
ltv_old = p_ltv_old - shock_ltv_old - shock_ltv_all;
...
shock_h=ro_shock_h*shock_h(-1)+eps_h;//shock to prefernce of unconstrsained
...
shock_ltv_old=ro_shock_ltv_old*shock_ltv_old(-1)+eps_ltv_old;
shock_ltv_all=ro_shock_ltv_all*shock_ltv_all(-1)+eps_ltv_all;
...
sell_h_tag = q*(h_o_tag(-1) - h_o_tag);
