...
variable_dir = fullfile(root_dir, 'fixed_no_internilize');
cd(variable_dir);


Baseline_fixed_wo_internilize_nc = combine_mods({ ...
    'A_Declaration.mod', 'B_Calibration_005.mod', 'C_Model.mod', ...
    'D_SS.mod', 'E_Shocks.mod', 'F_Irfs.mod'}, ...
    'Baseline_fixed_wo_internilize_nc.mod');
...
models = {Baseline_fixed_nc, Baseline_fixed_wo_internilize_nc};
leg_cel = {'Fixed-Rate with Internilize', 'Fixed-Rate without Internilize'};
...
endo_var   = cellstr(Baseline_fixed_wo_internilize_nc.M_.endo_names);
...
for var = 1:numel(Variabels_4_4)
    ...
    if ~any(strcmp(var_name, endo_var))
        error('Missing variable "%s" in variable baseline model (%s).', var_name, Baseline_fixed_wo_internilize_nc.M_.fname);
    end
    ...
    if ~isfield(Baseline_fixed_wo_internilize_nc.oo_.irfs, irf_field)
        error('Missing IRF field "%s" in variable baseline model (%s).', irf_field, Baseline_fixed_wo_internilize_nc.M_.fname);
    end
end
...
for shock = 1:numel(shock_list)
    ...
    for var = 1:nvars
        ...
        irf_var   = Baseline_fixed_wo_internilize_nc.oo_.irfs.(irf_field);
        ...
        mat_plot = [ ...
            ...
            irf_var(:)   + ss * Baseline_fixed_wo_internilize_nc.oo_.steady_state(find(strcmp(var_name, endo_var), 1))];
