...
//new equation for lagrangian multiplier for internilizeg , Nimrod 5/5/2026 
lagr_mu=lambda_tag - p_beta_tag*lambda_tag(+1)*RL + p_beta_tag * lagr_mu(+1) * (1 - ref)*(1-p_ltv_old) + lagr_Lambda * (1 - ref) * B_tag(-1)/( B_tag*B_tag) * (R + spread - RL(-1)) - p_beta_tag * lagr_Lambda(+1)*(1 - ref)/B_tag(+1) * (R(+1) + spread - RL);

//new equation for lagrangian multiplier for internilizeg , Nimrod 5/5/2026 
p_beta_tag*lambda_tag(+1)*B_tag + lagr_Lambda = p_beta_tag * lagr_Lambda(+1) * (1-ref)*B_tag / B_tag(+1) ;
...
DP_rent_nominal_annulized = ((DP_r/DP)*DP_cpi - 1 )*400;
DP_q_nominal_annulized = (q/q(-1)*DP_cpi - 1 )*400;
