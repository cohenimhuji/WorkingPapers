...
h=100; //total supply of houses in the economy
...
p_gamma_tag = 0.605; //0.62274; // 18.9.23 Nimrod. was updated to keep ownership to rent ratio 1.6 //0.65, 0.75 weight of ownership of constrained in the H (CES) function
...
p_ltv   = 0.67 ; // 18.9.23 Nimrod. before it was  0.55; //0.53 , LTV ratio in SS. Based on average LTV from 2011-2023 from Mortage file, sheet 18.
k       = 0.05*0; //adjustment costs (5% of the value of transaction)
tau     = 1 - 0.4 ; //ratio of unconstrained households in the economy 
teta    = 1/eps; //just definition used for futher purposes

par_con_aux=1+k+p_beta_tag*(k-1)-(1-beta_ratio-p_beta_tag*p_spread)*p_ltv; //usefull parameter
...
xi_cost_r= 0; // 311; // (based on Ilek and Rozenshroom, 2018) , should be bigger than xi_cost for rent to be more rigid than other goods.
...
p_r_q_ratio=1+k+p_beta*k-p_beta; //rent to price ratio in SS
ho_hr_ratio=p_gamma_au^(-1/teta); //ownership to rent ratio quantity of unconstrained in SS
ho_hr_ratio_tag=(p_r_q_ratio/(p_gamma_au_tag*par_con_aux))^(1/teta); //ownership to rent ratio quantity of constrained in SS
...
ro_shock_ltv=0.98*0; ////serial correlation of the LTV shock. High serial correlation means that agents expect that change in leverage will take place for a long time
